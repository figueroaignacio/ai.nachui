...
export function SuggestionCard({ title, onClick }: SuggestionCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="border-border/40 bg-card/40 hover:border-muted-foreground/20 hover:bg-muted/25 flex cursor-pointer flex-col gap-1 rounded-lg border p-3 text-left transition-colors duration-150"
    >
...
export function SidebarItem({ icon, label, active = false, onClick }: SidebarItemProps) {
  return (
    ...
    >
      {icon && <HugeiconsIcon icon={icon} className="size-4 shrink-0" size={16} />}
